...
export class ScrcpyViewProvider implements vscode.WebviewViewProvider {
  ...
  private async _onDidReceiveMessage(message: {
    ...
    switch (message.type) {
      ...
      case 'fileDrop':
        await this._handleFileDrop(message as unknown as {
          type: string;
          deviceId: string;
      ...
      case 'fileDropData':
        await this._handleFileDropData(message as unknown as {
          type: string;
          deviceId: string;
          name: string;
          data: string;
  ...
  /**
   * Handle file drop from webview
   */
  private async _handleFileDrop(message: {
    deviceId: string;
    files: Array<{ name: string; path: string; type: string }>;
  }): Promise<void> {
    if (!this._deviceManager || !this._view) {
      return;
    }
  ...
  /**
   * Handle file drop with base64 data from webview
   */
  private async _handleFileDropData(message: {
    deviceId: string;
    name: string;
    data: string;
  }): Promise<void> {
    ...
    const tempPath = path.join(tempDir, `scrcpy-drop-${Date.now()}-${name}`);
  ...
  private async _pairWifiDevice(): Promise<void> {
    ...
    vscode.window.showInformationMessage(vscode.l10n.t('Device paired successfully! Now connecting...'));
    ...
    const connectAddress = await vscode.window.showInputBox({
      ...
      prompt: vscode.l10n.t('Enter the connection address shown in Wireless debugging (not the pairing address)'),
      placeHolder: `${ip}:5555`,
      value: ip,
      validateInput: (value) => {
        if (!value) {
  ...
  private async _connectWifiDevice(): Promise<void> {
    const ipAddress = await vscode.window.showInputBox({
      ...
      validateInput: (value) => {
        ...
        const ipPortRegex = /^(\d{1,3}\.){3}\d{1,3}(:\d+)?$/;
        if (!ipPortRegex.test(value)) {
          return vscode.l10n.t('Enter a valid IP address (e.g., 192.168.1.100 or 192.168.1.100:5555)');
        }
    ...
    await vscode.window.withProgress(
      ...
      async () => {
        try {
          ...
          vscode.window.showInformationMessage(vscode.l10n.t('Connected to {0} over WiFi', deviceInfo.name));
        } catch (error) {
          const message = error instanceof Error ? error.message : String(error);
  ...
  private async _showDevicePicker(): Promise<void> {
    ...
    if (devices.length === 0) {
      vscode.window.showErrorMessage(vscode.l10n.t('No Android devices found. Please connect a device and enable USB debugging.'));
      return;
    }
    ...
    if (availableDevices.length === 0) {
      vscode.window.showInformationMessage(vscode.l10n.t('All available devices are already connected.'));
      return;
    }
